...
class ReportSubmitCompletePage extends StatelessWidget {
  const ReportSubmitCompletePage({super.key});

  // 리포트 공유 - 파일 다운로드 후 공유 기능
  Future<void> _shareReport(BuildContext context, int reportId) async {
    try {
      final file = await context.read<ReportViewModel>().downloadReport(reportId);
  ...
  Future<void> _showCompleteConfirmDialog(BuildContext context) async {
    ...
    const lightPink  = Color(0xFFFFF3F3);
  ...
  Widget build(BuildContext context) {
    ...
    const titleText = '오하이님의 8월 통합 리포트';
    const subtitleText = '완성된 파일을 다운받아 확인해보세요!';
